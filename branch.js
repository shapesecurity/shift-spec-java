...
`);
    ...
    let cl = `@SuppressWarnings("ConstantConditions")
    class ${name} extends `;
    if (isListMaybe) {
      cl += `IndexedBranch {
    ...
    public Maybe<? extends Node> step(Node node) {
        if (!(node instanceof ${typeName})) return Maybe.nothing();
        return ((${typeName}) node).${sanitize(a.name)}.index(index).orJust(Maybe.nothing());
    }
}`;
    } else if (isList) {
      cl += `IndexedBranch {
    ...
    public Maybe<? extends Node> step(Node node) {
        if (!(node instanceof ${typeName})) return Maybe.nothing();
        return Maybe.just(((${typeName}) node).${sanitize(a.name)});
    }
}`;
    }
